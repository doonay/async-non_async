...
async def get_page_data(session, page):
	...
	with open(f'models_{cur_time}_async.csv', 'w', encoding='utf-8') as file:
	...
	async with session.get(url=url, headers=headers) as response:
		...
		#print('https://daz4free.ru' + soup.find('a', class_='uk-position-cover').get('href'))
		...
		with open(f'models_{cur_time}_async.json', 'w', encoding='utf-8') as file:
		...
		for model in models_data:
			#print(model)
			with open(f'models_{cur_time}_async.csv', 'a', encoding='utf-8') as file:
				writer = csv.writer(file)
